...
Distributor.prototype._makeProject = function(project, buildParams) {
	...
	newProject = deepmerge(newProject, buildParams);
	...
	Steppy(
		...
		function(err, hasScmChanges) {
			if (params.withScmChangesOnly && !hasScmChanges) {
				logger.log(
					'Building of "%s" skipped due to no scm changes',
					project.name
				);
			...
			if (!params.queueQueued) {
				...
				if (queuedItem) {
					logger.log(
						'Building of "%s" skipped because it`s already queued',
						project.name
					);
					return callback(null, {});
				}
